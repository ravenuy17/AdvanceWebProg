...
function Signupteacher() {
  ...
  const postTeachersData = () => {
    if (
      ...
      axios.post(baseUrl + "addteachers", item).then(() => navigate("/"));
    } else {
      setValidation(true);
    }
  };
          ...
          <div className="lgnd1" style={{ height: "95vh", width: "70%" }}>
            ...
            <div style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "flex-start", height: "60px", justifyContent: "space-evenly" }}>
              <select onChange={(e) => setSpecialization(e.target.value)} style={{ width: "100%", height: 40, border: validation === true && specialization === "" ? "1px solid red" : "1px solid grey" }}>
                ...
                <option>Other</option>
                {courseData.map((i) => (
                  <option key={i.courseName}>{i.courseName}</option>
                ))}
              </select>
            </div>
            <div style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "flex-start", height: "60px", justifyContent: "space-evenly" }}>
            ...
            <div style={{ width: "100%", display: "flex", flexDirection: "row", alignItems: "flex-start", height: "60px", justifyContent: "space-between" }}>
              ...
              <input onChange={(e) => setFrom(e.target.value)} style={{ width: "25%", height: 40, border: validation === true && from === "" ? "1px solid red" : "1px solid grey" }} type="time" />
              <input onChange={(e) => setTo(e.target.value)} style={{ width: "25%", height: 40, border: validation === true && to === "" ? "1px solid red" : "1px solid grey" }} type="time" />
            ...
            <label>
              Already have an account? <span onClick={() => navigate("/")} style={{ color: "#8b5fb3" }}>Login</span>
            </label>
          </div>
        </div>
        <div className="lgnd1" style={{ height: "100vh", width: "50%" }}>
          <div style={{ height: "100vh", width: "100%", backgroundColor: "black", display: "flex", justifyContent: "center", textAlign: "center", alignItems: "center", color: "white" }}>
